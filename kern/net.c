...
net_rx(void *pkt, int len)
{
	...
	else if (h->type == NET_MIGRP){
		net_rxmigrp(pkt);
	}
	else if (h->type == NET_PULLRQ){
		net_rxpullrq(pkt);
	}
	else if (h->type == NET_PULLRP){
...
net_tick()
{
	...
	proc * cur = net_migrlist;
	while(cur){
		net_txmigrq(cur);
		cur = cur->migrnext;
	}

	//ticking to call net_rxpullrq
	proc *npull = net_pulllist;
	while(npull){
		net_txpullrq(npull);
		npull = npull->pullnext;
	}
...
net_migrate(trapframe *tf, uint8_t dstnode, int entry)
{
	...
	p->migrnext = NULL;
	p->migrdest = dstnode;
	p->rrpdir = (uint32_t) p->pdir;
	spinlock_release(&p->lock);
	
	spinlock_acquire(&net_lock);
	if (net_migrlist) {
		proc * last = net_migrlist;
		while(last->migrnext) last = p->migrnext;
		last->migrnext = p;
	} else {
		net_migrlist = p;	
	}
...
net_txmigrq(proc *p)
{
	...
	net_migrq r;
	net_ethsetup(&r.eth, p->migrdest);
	
	r.type = NET_MIGRQ;
	r.home = p->home;
	r.pdir = RRCONS(net_node, mem_phys(p->pdir), 0);
	r.save = p->sv;
	
	net_tx(&r, sizeof(net_migrq), NULL, 0);
}
...
net_txmigrp(uint8_t dstnode, uint32_t prochome)
{
	...
	net_migrp r;
	net_ethsetup(&r.eth, dstnode);
	
	r.type = NET_MIGRP;
	r.home = prochome;
	
	net_tx(&r, sizeof(net_migrp), NULL, 0);
}
...
void net_rxmigrp(net_migrp *migrp)
{
	...
	if (!net_migrlist) return;
	
	proc* cur = net_migrlist;
	
	if (cur->migrdest == msgsrcnode && cur->home == migrp->home){
		//cprintf("found it 1st\n");
		spinlock_acquire(&cur->lock);
		spinlock_acquire(&net_lock);
		cur->state = PROC_AWAY;
		net_migrlist = cur->migrnext;
		spinlock_release(&net_lock);
	...
	proc* last = NULL;
	while (cur->migrdest != msgsrcnode || cur->home != migrp->home){
			last = cur;
			cur = cur->migrnext;
			if (cur == NULL) return; //Didn't find
	}
	//cprintf("found it nth\n");
	spinlock_acquire(&cur->lock);
	cur->state = PROC_AWAY;
	last->migrnext = cur->migrnext;
	spinlock_release(&cur->lock);
}
...
net_txpullrq(proc *p)
{
	...
	net_pullrq r;
	net_ethsetup(&r.eth, RRNODE(p->pullrr));
	r.type = NET_PULLRQ;
	r.rr = p->pullrr;
	r.pglev = p->pglev;
	r.need = ~ p->arrived & 7;
	net_tx(&r, sizeof(net_pullrq), NULL, 0);
}
...
net_rxpullrq(net_pullrq *rq)
{
	...
	if (rq->need & 3) net_txpullrp(rqnode, rr, rq->pglev, 2, pg);
...
net_txpullrp(uint8_t rqnode, uint32_t rr, int pglev, int part, void *pg)
{
	...
	if (pglev > 0) {
		...
		for (i; i < nrrs; i++){
			if (PGADDR(pt[i]) == PTE_ZERO){
				rrs[i] = RRCONS(net_node, 0, RR_RW & pt[i]);
			}
			else {
				pageinfo* pi = mem_phys2pi(PGADDR(pt[i]));
				if (pi->home == 0 || pglev == PGLEV_PDIR) {
					rrs[i] = RRCONS(net_node, PGADDR(pt[i]), RR_RW & pt[i]);
				}
				else {
					rrs[i] = pi->home;
				}
...
net_pullpte(proc *p, uint32_t *pte, int pglevel)
{
	...
	else if (RRNODE(rr) == net_node) {
		*pte = RRADDR(rr) | (rr & RR_RW) | PTE_P | PTE_U | PTE_W; //TODO
		return true;
	}
	else if (mem_rrlookup(rr)) {
		pageinfo* pi = mem_rrlookup(rr);
		*pte = PGADDR(mem_pi2phys(pi)) | PTE_P | PTE_U | SYS_RW;
		if (pglevel) *pte |= PTE_W;
		return true;
	}
	else {
		...
		pi->refcount++;
		mem_rrtrack(rr, pi);
		net_pull(p, *pte, mem_pi2ptr(pi), pglevel);
		return false;
	}
}
