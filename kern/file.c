...
file_initroot(proc *root)
{
	...
	for (i = 0; i < ninitfiles; i++) {
		int fileno = i + FILEINO_GENERAL;
		uintptr_t fsize = initfiles[i][2] - initfiles[i][1];
		void *fdat = FILEDATA(fileno);
		strcpy(files->fi[fileno].de.d_name, initfiles[i][0]);
		files->fi[fileno].dino = FILEINO_ROOTDIR;
		files->fi[fileno].mode = S_IFREG;
		files->fi[fileno].size = fsize;
		pmap_setperm(root->pdir, (uintptr_t)fdat, PTSIZE, SYS_READ | SYS_WRITE);
		memmove(fdat, initfiles[i][1], fsize);
	}
