...
fileino_read(int ino, off_t ofs, void *buf, size_t eltsize, size_t count)
{
	...
	while (ofs >= fi->size){
		if (fi->mode & S_IFPART) sys_ret();
		else return 0;
	}

	size_t sz = MIN(eltsize * count, fi->size - ofs);
	memmove(buf, FILEDATA(ino) + ofs, sz);
	return sz / eltsize;
...
fileino_write(int ino, off_t ofs, const void *buf, size_t eltsize, size_t count)
{
	...
	size_t sz = eltsize * count;
	if (sz + ofs >= FILE_MAXSIZE) {
		errno = EFBIG;
		return -1;
	}

	if (fi->size == 0)
		sys_get(SYS_PERM | SYS_READ | SYS_WRITE, 0, NULL, NULL, FILEDATA(ino),
				PTSIZE);

	memmove(FILEDATA(ino) + ofs, buf, sz);
	fi->size = MAX(fi->size, sz + ofs);
	return sz / eltsize;
