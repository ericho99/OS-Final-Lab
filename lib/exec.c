...
exec_copyargs(char *const argv[])
{
	...
	intptr_t new_esp = VM_SCRATCHLO + PTSIZE;
	int diff = esp - new_esp;
	...
	int i;
	for (i = 0; i < argc; i++){
		new_esp -= strlen(argv[i]) + 1;
		strcpy((char *) new_esp, argv[i]);
		new_argv[i] = (char *) new_esp + diff;
	}
	
	new_esp -= sizeof(int);
	*(intptr_t *) new_esp = 0;
	
	new_esp -= sizeof(int) * argc;
	strcpy((char *) new_esp, (char *) new_argv);

	new_esp -= sizeof(int);
	*(intptr_t *) new_esp = new_esp + diff + sizeof(int);
	
	new_esp -= sizeof(int);
	*(intptr_t *) new_esp = argc;
	
	// Copy the stack into its correct position in child 0.
	sys_put(SYS_COPY, 0, NULL, (void*)VM_SCRATCHLO,
		(void*)VM_STACKHI-PTSIZE, PTSIZE);

	//cprintf("exec_copyargs() complete\n");
	return new_esp + diff;
}
