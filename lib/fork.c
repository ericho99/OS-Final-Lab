...
reconcile_inode(pid_t pid, filestate *cfiles, int pino, int cino)
{
	...
	// If conflict can be merged
	if (pfi->ver == rver && cfi->ver == rver 
		&& pfi->size > rlen && cfi->size > rlen){
		return reconcile_merge(pid, cfiles, pino, cino);
	}
	
	// If neither parent nor child changed the file
	else if (pfi->ver == rver && pfi->size == rlen
		&& cfi->ver == rver && cfi->size == rlen){
		//cprintf("reconcile - both versions unchanged\n");
		return 1;
	}
	// If only child changed
	else if (pfi->ver == rver && pfi->size == rlen){
		//cprintf("reconcile - only child changed\n");
		//Update parent
		pfi->ver = cfi->ver;
		pfi->size = cfi->size;
		pfi->mode = cfi->mode;
	...
	// If only parent changed
	else if (cfi->ver == rver && cfi->size == rlen){
		//cprintf("reconcile - only parent changed\n");
		//Update child
		cfi->ver = pfi->ver;
		cfi->size = pfi->size;
		cfi->mode = pfi->mode;
		...
		//Copy file
		sys_get(SYS_COPY, pid, NULL, FILEDATA(pino), FILEDATA(cino), PTSIZE);
		
		return 1;
	}
	//If conflict
	else {
		//cprintf("reconcile - conflict\n");
		cfi->mode |= S_IFCONF;
		pfi->mode |= S_IFCONF;
		return 0;
	}
...
reconcile_merge(pid_t pid, filestate *cfiles, int pino, int cino)
{
	...
	// Copy child file into scratch space
	char * cstart = (char*) VM_SCRATCHHI - PTSIZE;
	sys_get(SYS_COPY,pid, NULL, FILEDATA(cino), cstart, PTSIZE);
	
	// Find what to append for each file
	char * cnew = cstart + cfi->rlen;
	char * cend = cstart + cfi->size;
	int csize = cfi->size - cfi->rlen;
	
	char * pstart = FILEDATA(pino);
	char * pnew = pstart + cfi->rlen;
	char * pend = pstart + pfi->size;
	int psize = pfi->size - cfi->rlen;
	
	// Copy child's new stuff to end of parent
	memcpy(pend, cnew, csize);
	pfi->size += csize;
	
	// Copy parent's new stuff to end of child, copy back from scratch space
	memcpy(cend, pnew, psize);
	cfi->size += psize;
	cfi->rlen = cfi->size;
	sys_put(SYS_COPY,pid, NULL, cstart, FILEDATA(cino), PTSIZE);
